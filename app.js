...
class SaveChat {
    ...
    async sendEmailVerification(email, code) {
        try {
            ...
            if (this.emailjsInitialized && emailjs) {
            ...
            console.log('EmailJS not configured, falling back to demo mode');
        }
    ...
    showDemoCode(code, destination, type) {
        ...
        demoNotification.className = 'fixed top-20 left-1/2 transform -translate-x-1/2 bg-yellow-100 dark:bg-yellow-900 border border-yellow-400 text-yellow-800 dark:text-yellow-200 px-6 py-4 rounded-lg shadow-lg z-50 max-w-md';
        demoNotification.innerHTML = `
            <div class="flex items-start">
                ...
                <div class="ml-3">
                    <h3 class="text-sm font-medium">Demo Mode - ${type} Verification</h3>
                    <div class="mt-2 text-sm">
                        <p><strong>To:</strong> ${destination}</p>
                        <p><strong>Code:</strong> <span class="font-mono text-lg bg-yellow-200 dark:bg-yellow-800 px-2 py-1 rounded">${code}</span></p>
                        <p class="text-xs mt-2">This is a demo. In production, this would be sent via ${type}.</p>
                    </div>
                </div>
